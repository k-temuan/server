...
const { getToken } = require('../helpers/jwt') // ganti jadi getToken
...
beforeAll((done) => {
  ...
  User.create(input)
    ...
    .then((user) => {
      access_token = getToken({ id: user.id }) // berubah jadi getToken
  ...
  describe('success process', () => {
    ...
    describe('create event', () => {
      it('should create a new event', (done) => {
        request(app)
          ...
          .field('category', 'game') // berubah jadi "game"
          .field('description', 'Mabar DOTA 2 area Jakarta')
          .field('max_attendees', 10)
          .field('location', 'Jakarta')
          .field('date_time', '2020-05-10 11:00') // form data ga bisa nerima object
          .field('tags', tags) // belom ke-handle
          .end((err, res) => {
            expect(res.statusCode).toEqual(201)
            expect(res.body).toHaveProperty('event')
            expect(res.body.event).toHaveProperty('name', 'Mabar DOTA 2')
            expect(res.body.event).toHaveProperty('category', 'game') // berubah jadi "game"
            ...
            expect(res.body.event).toHaveProperty('EventTags') // belom ke-handle
      ...
      it('should fetch a single event', (done) => {
        request(app)
          ...
          .end((err, res) => {
            ...
            expect(res.body.event).toHaveProperty('location', 'Jakarta')
            expect(res.body.event).toHaveProperty('date_time')
            expect(res.body.event).toHaveProperty('EventTags') // diganti jadi EventTags di dalam sini baru ada Tag
            done()
          })
      })
    })
    ...
    describe('update event with image', () => {
      it('should update a event with image', (done) => {
        request(app)
          .patch(`/events/${eventId}`) // diganti jadi patch
          .set('access_token', access_token)
          .attach('image', eventImage, {
            contentType: 'application/octet-stream',
          })
          .field('name', 'Mabar DOTA 2')
          .field('category', 'game') // ganti jadi "game"
          .field('description', 'Mabar DOTA 2 area Jakarta Barat')
          .field('max_attendees', 100)
          .field('location', 'Jakarta Barat')
          .field('date_time', '2020-05-10 10:00') // ganti jadi date manual
          .field('tags', tags) // belom ke-handle
          .end((err, res) => {
            expect(res.statusCode).toEqual(200)
            expect(res.body).toHaveProperty('event')
            expect(res.body.event).toHaveProperty('name', 'Mabar DOTA 2')
            expect(res.body.event).toHaveProperty('category', 'game') // ganti jadi game
            expect(res.body.event).toHaveProperty(
              'description',
    ...
    describe('update event without image', () => {
      // maksudnya image jadi "" atau image tetep seperti yg pertama??
      it('should update event without image', (done) => {
        request(app)
          .patch(`/events/${eventId}`)
          .set('access_token', access_token)
          .field('name', 'Mabar DOTA 2')
          .field('category', 'game') // 'game'
          .field('description', 'Mabar DOTA 2 area Jakarta Barat')
          .field('max_attendees', 100)
          .field('location', 'Jakarta Barat')
          .field('date_time', '2020-05-10 10:00') // ganti jadi date manual
      ...
      it('should return validation error', (done) => {
        request(app)
          ...
          .end((err, res) => {
            ...
            expect(res.body).toHaveProperty('errors', expect.any(Array))
            expect(res.body.errors).toContain('Event name cannot be empty') // ganti error message
            expect(res.body.errors.length).toBeGreaterThan(0)
            done()
          })
      })
